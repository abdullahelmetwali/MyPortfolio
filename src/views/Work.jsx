    ...
    <>
      ...
      <section className="grid grid-cols-3 content mob:fadeUp gap-4 my-24 px-10 mob:flex mob:flex-col mob:px-5">
        {WorkProjects.map((project, index) => (
          <NavLink key={index} to={`/work/${project.projectName.replaceAll(" ", "").toLowerCase()}`} className={`box-${index + 1} bg-[#0f0e0e] textAcorn text-center rounded-xl p-4 fadeUp`} style={{ gridArea: `box-${index + 1}` }}>
